...
def create_presentation():
    """Accept JSON spec, render to .pptx"""
    try:
        spec = request.get_json()
        if not spec:
            return jsonify({"error": "No JSON"}), 400
 
        filename = spec.get('output_filename', 'presentation.pptx')
        if not filename:
            return jsonify({"error": "output_filename required"}), 400
 
        logger.info(f"Building presentation: {filename}")
        pptx_binary = presentation_builder.build(spec)
 
        return send_file(
            io.BytesIO(pptx_binary),
...
def root():
    return jsonify({"service": "WRLD Document Generator", "version": "2.3", "formats": ["docx", "pptx"], "endpoints": ["/create-template", "/create-presentation"]}), 200
